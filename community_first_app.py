...
def safe_logo(path, caption=None):
    """Render a logo if it exists and is a non-empty, valid image; otherwise show a text placeholder."""
    try:
        ...
        # If PIL can't decode or any other error, fall back to text
        st.markdown(f"**{caption or 'Logo'}**")
...
if latest is not None and "date" in latest.columns and not latest.empty:
    try:
        ...
        as_of = None
# store in session_state for PDF tab
...
with tab5:
    ...
    if st.button("Generate PDF"):
        ...
        # logos (optional placeholders) — ignore failures
...
st.markdown(f"<hr style='border:1px solid {BORDER};'>", unsafe_allow_html=True)
st.caption("© {} Minnesota Food Insecurity EWS — Branded Main (robust logos)".format(pd.Timestamp.today().year))
